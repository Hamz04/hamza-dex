...
interface IArcSwapCallee {
    function arcSwapCall(address sender, uint256 amount0Out, uint256 amount1Out, bytes calldata data) external;
}
 ...
 */
contract ArcSwapPair is ArcSwapERC20 {
    ...
    modifier lock() {
        require(unlocked, "ArcSwap: LOCKED");
        unlocked = false;
        _;
        unlocked = true;
    }
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external lock {
        ...
        uint256 balance1;
        {
            ...
            if (data.length > 0) IArcSwapCallee(to).arcSwapCall(msg.sender, amount0Out, amount1Out, data);
        ...
        require(amount0In > 0 || amount1In > 0, "ArcSwap: INSUFFICIENT_INPUT_AMOUNT");
        {
            ...
            require(balance0Adjusted * balance1Adjusted >= uint256(_reserve0) * _reserve1 * 1000**2, "ArcSwap: K");
        }
        _update(balance0, balance1, _reserve0, _reserve1);
